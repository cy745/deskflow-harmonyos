...
- **键盘映射**覆盖常用键（含字母/数字/F1-F12/方向/编辑/修饰/标点/小键盘），个别媒体、厂商键未映射。
- 需先手动点连接；暂无开机自启、后台驻留。

## 可靠性特性

- ✅ **断线自动重连**：连接意外断开（服务端重启、网络抖动）后按 3s 间隔自动重连，直到成功。
- ✅ **错误分类**：可重连的错误（连接丢失、协议错误、服务端繁忙）自动重连；
  不可重连的错误（服务端主动关闭 CBYE、版本不兼容 EICV/Hello 主版本不匹配）停止重连并提示。
